...
(function () {
  ...
  function isModalVisible(el) {
    ...
    /* 親が display:none などで非表示のとき、子の display:flex だけでは開いていない */
  ...
  function hasOpenModal() {
    return getVisibleSheetRoots().length > 0;
  }
  ...
  function getScheduleHeader() {
    ...
    return scheduleSection ? scheduleSection.querySelector('.schedule-header') : null;
  }

  function getScheduleSearchWrap() {
    var header = getScheduleHeader();
    var child;
    var i;
    if (header) {
      for (i = 0; i < header.children.length; i++) {
        child = header.children[i];
        if (child.classList && child.classList.contains('search')) return child;
      }
    }
    var topbar = document.querySelector('.topbar');
    return topbar ? topbar.querySelector('.search') : null;
  }

  function getScheduleWarnBtn() {
    var header = getScheduleHeader();
    var topbar = document.querySelector('.topbar');
    var selector = '.schedule-web-reception-btn, .warn-btn';
    var nodes;
    var i;
    if (header) {
      nodes = header.querySelectorAll(selector);
      for (i = 0; i < nodes.length; i++) {
        if (nodes[i].parentNode === header) return nodes[i];
      }
    }
    return topbar ? topbar.querySelector(selector) : null;
  }
  ...
  function restoreScheduleToolbarToTopbar(topbar, searchWrap, warnBtn, refreshBtn) {
    ...
    if (refreshBtn) {
      var refreshControls = document.querySelector('.schedule-section .schedule-refresh-controls');
      var dateControls = document.querySelector('.schedule-section .date-controls');
      var refreshContainer = refreshControls || dateControls;
      if (refreshContainer && refreshBtn.parentNode !== refreshContainer) {
        ...
        } else if (refreshControls) {
          refreshControls.insertBefore(refreshBtn, refreshControls.firstChild);
        } else if (dateControls) {
  ...
  function syncReservationScheduleToolbar() {
    ...
    var searchWrap = getScheduleSearchWrap();
    var warnBtn = getScheduleWarnBtn();
  ...
  function enhancePageCancelButtons() {
    document.querySelectorAll('.search-form-actions .cancel-btn, .search-form-actions button[onclick*="closeShiftPage"], .sort-actions .form-btn-cancel, .sort-actions button[onclick*="goBack"], .sort-actions button[onclick*="closeShiftPage"]').forEach(function (btn) {
  ...
  function getModalOverlayRoot(el) {
    if (!el) return null;
    return el.closest('.popup-overlay, .list-popup-overlay, .reservation-modal, .block-modal, .reservation-detail-modal, dialog');
  }

  function isModalOverlayVisible(overlay) {
    if (!overlay) return true;
    if (overlay.hidden) return false;
    if (overlay.open === false) return false;
  ...
  function isVisibleModalActionButton(btn) {
    ...
    var overlay = getModalOverlayRoot(btn);
    if (overlay && !isModalOverlayVisible(overlay)) {
      return true;
    }
  ...
  function bindModalOverflowMenu(actions) {
    ...
    if (toggle.dataset.spOverflowToggleBound === '1') return;
    toggle.dataset.spOverflowToggleBound = '1';
  ...
  function ensureModalOverflowGlobalHandlers() {
    ...
    document.addEventListener('click', function (e) {
      if (e.target.closest('.modal-actions-overflow, .modal-overflow-toggle, .modal-overflow-menu')) return;
      document.querySelectorAll('.modal-actions--sp-footer .modal-overflow-menu:not([hidden]), .modal-actions-new.modal-actions--sp-footer .modal-overflow-menu:not([hidden])').forEach(function (menu) {
  ...
  function isDetailManualOverflowActions(actions) {
    return actions.classList.contains('modal-actions--detail') &&
      !!actions.querySelector(':scope > .modal-actions-overflow');
  }
  ...
  function prepareModalFooterActions(actions) {
    ...
    if (isDetailManualOverflowActions(actions)) {
  ...
  function transformModalFooterActions(actions) {
    ...
    if (hasManualOverflow) {
      if (isDetailManualOverflowActions(actions) && actions.dataset.spOverflowTransformed === '1') {
        return;
      }
