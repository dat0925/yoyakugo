$(function () {
    // カレンダー表 tbody をデータからレンダリング（翻訳対象外のため JS で描画）
    const renderCalendarTable = () => {
        const tbody = $('#calendar-tbody');
        if (!tbody.length) return;

        const calendarBaseDate = '2025-08-01';
        const calendarDays = 14;
        const timeSlots = [
            { time: '10:00', group: 'morning' },
            { time: '10:30', group: 'morning' },
            { time: '11:00', group: 'morning' },
            { time: '11:30', group: 'morning' },
            { time: '12:00', group: 'afternoon' },
            { time: '12:30', group: 'afternoon' },
            { time: '13:00', group: 'afternoon' },
            { time: '13:30', group: 'afternoon' },
            { time: '14:00', group: 'afternoon' },
            { time: '14:30', group: 'afternoon' },
            { time: '15:00', group: 'afternoon' },
            { time: '15:30', group: 'afternoon' },
            { time: '16:00', group: 'afternoon' },
            { time: '16:30', group: 'afternoon' },
            { time: '17:00', group: 'afternoon' },
            { time: '17:30', group: 'afternoon' },
            { time: '18:00', group: 'evening' },
            { time: '18:30', group: 'evening' }
        ];
        // 受付不可スロット（data-date 形式 "YYYY-MM-DD-HH:mm"）
        const fullSlots = new Set([
            '2025-08-05-10:00', '2025-08-05-10:30', '2025-08-05-11:00', '2025-08-05-11:30', '2025-08-05-12:00', '2025-08-05-12:30', '2025-08-05-13:00',
            '2025-08-06-10:00', '2025-08-06-10:30', '2025-08-06-11:00', '2025-08-06-11:30', '2025-08-06-12:00', '2025-08-06-12:30', '2025-08-06-13:00', '2025-08-06-13:30',
            '2025-08-07-10:00', '2025-08-07-10:30', '2025-08-07-11:00', '2025-08-07-11:30', '2025-08-07-12:00', '2025-08-07-12:30', '2025-08-07-13:00', '2025-08-07-13:30', '2025-08-07-14:00', '2025-08-07-14:30'
        ]);

        const base = new Date(calendarBaseDate);
        const rows = [];
        timeSlots.forEach(function (slot) {
            let cells = '<tr data-group="' + slot.group + '"><th>' + slot.time + '</th>';
            for (let d = 0; d < calendarDays; d++) {
                ...
                const dateKey = y + '-' + m + '-' + day + '-' + slot.time;
                const isFull = fullSlots.has(dateKey);
                const cls = isFull ? 'full' : 'accepting';
                const dataDate = ' data-date="' + dateKey + '"';
                const input = isFull
                    ? '<input type="radio" name="date_time" disabled>×'
                    : '<input type="radio" name="date_time">◎';
                cells += '<td class="' + cls + '"' + dataDate + '>' + input + '</td>';
            }
            cells += '</tr>';
            rows.push(cells);
        });
        tbody.html(rows.join(''));
        if ($('html').attr('lang') !== 'ja' && $('.step02 .calendar_wrap .calendar').length) {
    ...
    const initLanguageSwitcher = () => {
        ...
        languageDropdown.find('.language-option').on('click', function (e) {
            ...
            // 異なる言語の場合のみ遷移
            if (langCode !== currentLangCode) {
                const url = window.location.href.replace('/' + currentLangCode, '/' + langCode);
    ...
    if ($('html').attr('lang') !== 'ja') {
        // h1 class="title"を検索しvalueをshopName_ja(title_value)に変更する
        if ($('h1.title').length) {
            $('h1.title').text(shopName_ja+'('+$('h1.title').html()+')');
        }
        if ($('.step01').length) {
